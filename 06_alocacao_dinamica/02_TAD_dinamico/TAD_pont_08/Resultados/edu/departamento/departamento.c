...
{
    tDepartamento* dep = malloc(sizeof(tDepartamento));

    dep->c1 = NULL;
    dep->c2=NULL;
    dep->c3=NULL;
    dep->diretor=NULL;
    dep->m1 = dep->m2 = dep->m3 = -1;
    dep->media_geral = 0;
    dep->nome = NULL;
...
void PreencheDadosDepartamento(tDepartamento* depto, char* c1, char* c2, char* c3, char* nome, int m1, int m2, int m3, char* diretor)
{
    ...
    int tam = strlen(c1) + 1;
    depto->c1 = malloc(tam*sizeof(char));

     tam = strlen(c2) + 1;
    depto->c2 = malloc(tam*sizeof(char));

     tam = strlen(c3) + 1;
    depto->c3 = malloc(tam*sizeof(char));

    tam = strlen(nome) + 1;
    depto->nome = malloc(tam*sizeof(char));

    tam = strlen(diretor) + 1;
    depto->diretor = malloc(tam*sizeof(char));
