...
tMonitorador* criaMonitorador()
{
    ...
    m->qnt = 0;
    m->evento = malloc(sizeof(tEvento*));
...
{
    for(int i=0;i<m->qnt;i++)
    {
        m->evento[i]->libera(m->evento[i]->evento);
        free(m->evento[i]);
    }
    free(m->evento);
    free(m);
...
{
    m->evento[m->qnt] = malloc(sizeof(tEvento));
    m->evento[m->qnt]->evento = e;
    m->evento[m->qnt]->identifica = funcId;
    m->evento[m->qnt]->processa = funcPE;
    m->evento[m->qnt]->libera = funcLE;

    m->qnt++;
    m->evento = realloc(m->evento,(m->qnt+1)*sizeof(struct evento*));
}
...
void iniciaMonitoramentotMonitorador(tAmbiente* amb, tMonitorador* m)
{
    while(1)
    {
        ...
        for(int i=0;i<m->qnt;i++)
        {
            if(m->evento[i]->identifica(m->evento[i]->evento,cod))
            {
                m->evento[i]->processa(amb,m->evento[i]->evento,cod);
                saida =0;
            }
        }
